...
impl ToolHandler for ToolSearchHandler {
    ...
    async fn handle(
        ...
        let results = self.search_result_entries(query, requested_limit);
...
mod tests {
    ...
    fn computer_use_tool_search_uses_larger_limit() {
        ...
        let results = handler.search_result_entries("computer use", /*requested_limit*/ None);
        ...
        let explicit_results =
            handler.search_result_entries("computer use", /*requested_limit*/ Some(100));

        assert_eq!(explicit_results.len(), COMPUTER_USE_TOOL_SEARCH_LIMIT);
    }
    ...
    fn non_computer_use_query_keeps_default_limit_with_computer_use_tools_installed() {
        ...
        let results = handler.search_result_entries("calendar", /*requested_limit*/ None);
        ...
        let explicit_results =
            handler.search_result_entries("calendar", /*requested_limit*/ Some(100));

        assert_eq!(explicit_results.len(), TOOL_SEARCH_DEFAULT_LIMIT);
    }
